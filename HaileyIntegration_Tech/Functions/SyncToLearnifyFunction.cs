...
public sealed class SyncToLearnifyFunction(
    ILearnifyService learnifyService,
    IEmployeeMappingService mappingService,
    ILogger<SyncToLearnifyFunction> logger)
{
    [Function(nameof(SyncToLearnifyFunction))]
    public async Task<HttpResponseData> Run(
        [HttpTrigger(AuthorizationLevel.Function, "post", Route = "sync/learnify")] HttpRequestData req,
        CancellationToken ct)
    {
        ...
        {
            logger.LogWarning("Invalid Learnify sync request: {Message}", ex.Message);
        ...
        logger.LogInformation("Syncing employee {EmployeeNumber} to Learnify", employee.EmploymentNumber);

        var canonical = mappingService.MapToCanonical(employee, ChangeType.Update);
        var result = await learnifyService.SyncEmployeeAsync(canonical, ct);
